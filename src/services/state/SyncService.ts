...
export class RealTimeSyncManager {
  ...
  private scheduleReconnect(): void {
    ...
    const delay = this.config.reconnectInterval * 2 ** (this.reconnectAttempts - 1); // Exponential backoff
  ...
  private setupDefaultConflictResolvers(): void {
    ...
    this.setConflictResolver('machine', (local: MachineState, remote: MachineState) => (local.lastUpdate > remote.lastUpdate ? local : remote));
