...
import { PluginStateRecord } from '../database/types';
import { Plugin, PluginUpdate, RegistryConfig, PluginContextType } from './types';
...
export const PluginProvider: React.FC<PluginProviderProps> = ({ children }) => {
  ...
  const fetchMarketplacePlugins = async (): Promise<Plugin[]> => {
    return await PluginService.fetchMarketplacePlugins(registryConfig || undefined);
  };
  ...
  const checkDependencies = async (plugin: Plugin): Promise<boolean> => {
    return await PluginService.checkDependencies(plugin, plugins);
  };
  ...
  const exportPlugins = async (): Promise<string> => {
    return await PluginService.exportPlugins(plugins);
  };
  ...
  const getStandalonePlugins = () => {
    return PluginService.getStandalonePlugins(plugins);
  };

  const getPluginAPI = () => {
    return PluginService.getPluginAPI();
  };
